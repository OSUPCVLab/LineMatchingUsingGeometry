...
function H = homography2d1(varargin)
    
    [x1, x2] = checkargs(varargin(:));

%     x1=cross(x1(1:3,:),x1(4:6,:));    T1=eye(3);
%     x2=cross(x2(1:3,:),x2(4:6,:));    T2=eye(3);
...
%     % 1. This section of building H is based on single points correspondences
  if size(x1,1)==3
      Npts = length(x1);
      A = zeros(3*Npts,9);
      O = [0 0 0];
...
function [x1, x2] = checkargs(arg)
	...
	elseif size(x1,1) ~= 6
	    error('x1 and x2 must be 6xN');
	end
	
    elseif length(arg) == 1
	if size(arg{1},1) ~= 12
	    error('Single argument x must be 12xN');
	else
	    x1 = arg{1}(1:6,:);
	    x2 = arg{1}(7:12,:);
	end
