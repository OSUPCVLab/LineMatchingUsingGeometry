...
function [newpts, T,Ts] = normalise2dpts(pts)

%% This section is only when using points representation (just points, or line end-points)
if size(pts,1) == 6
    
    
    % Find the indices of the points that are not at infinity
    finiteind = find((abs(pts(3,:)) & abs(pts(6,:))) > eps);
    ...
    pts(4,finiteind) = pts(4,finiteind)./pts(6,finiteind);
    pts(5,finiteind) = pts(5,finiteind)./pts(6,finiteind);
    pts(6,finiteind) = 1;
    
    c = mean([pts(1:2,finiteind) pts(4:5,finiteind)],2);    % Centroid of finite points
%     c = mean(pts(1:2,finiteind)')';            % Centroid of finite points
    ...
    newp(4,finiteind) = pts(4,finiteind)-c(1); 
    newp(5,finiteind) = pts(5,finiteind)-c(2);

    
    dist = sqrt((newp(1,finiteind).^2 + newp(4,finiteind).^2)./2 + ...
                (newp(2,finiteind).^2 + newp(5,finiteind).^2)./2);
    ...
    newpts = [T*pts(1:3,finiteind);T*pts(4:6,finiteind)];
end    
